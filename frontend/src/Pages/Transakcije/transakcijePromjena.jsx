...
const TransakcijePromjena = () => {
  const { transakcijaId } = useParams();
  ...
  useEffect(() => {
    fetch(`API_URL/transakcije/${transakcijaId}`) // Dohvati postojeću transakciju
      .then(response => response.json())
      .then(data => {
        setTransakcija(data);
        setKolicina(data.kolicina);
        setKriptoId(data.kripto_id);
        setNaknada(data.naknada);
      })
  ...
  const handleSubmit = (e) => {
    ...
    const updatedTransakcija = {
      kolicina,
      kripto_id: kriptoId,
      naknada
    };

    fetch(`API_URL/transakcije/${transakcijaId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(updatedTransakcija)
    })
      .then(response => response.json())
      .then(data => {
        console.log('Transakcija uspješno ažurirana', data);
      })
      .catch(error => console.log(error));
  };

  if (!transakcija) {
    return <div>Učitavanje transakcije...</div>;
  }

  return (
    <div>
      <h2>Promjeni transakciju</h2>
...
export default TransakcijePromjena;
